...
- Always-running local backend service (Python/FastAPI) that:
  ...
  - polls Sonar state
  - persists state to `tools/native_windows_dashboard_state.json`
...
```text
applications/native_windows_dashboard/
  backend/
    main.py
    requirements.txt
...
```powershell
python -m pip install -r applications/native_windows_dashboard/backend/requirements.txt
```
...
```powershell
dotnet build applications/native_windows_dashboard/frontend/NativeDashboard/NativeDashboard.csproj
dotnet run --project applications/native_windows_dashboard/frontend/NativeDashboard/NativeDashboard.csproj
```

The frontend auto-starts the Python backend process (`applications/native_windows_dashboard/backend/main.py`).
...
- State file fallback path:
  - `tools/native_windows_dashboard_state.json`
