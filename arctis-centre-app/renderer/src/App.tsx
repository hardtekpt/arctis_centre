...
export default function App() {
  ...
  if (windowMode === "about") {
    return (
      <main className="window-base app-shell standalone-page">
        <AboutPage logs={logs} lastStatus={status} lastError={error} />
        <div className="standalone-actions">
          <button className="button" onClick={() => actions.closeCurrentWindow()}>
            Close
          </button>
  ...
  return (
    <main className="window-base app-shell">
      <header className="titlebar">
        <div className="titlebar-left">
          ...
          <div className="topbar-battery-wrap">
            <BatteryBadge kind="headset" percent={state.headset_battery_percent ?? 0} showPercent={settings.showBatteryPercent} />
            <BatteryBadge kind="charging" percent={state.base_battery_percent ?? 0} showPercent={settings.showBatteryPercent} />
          </div>
        </div>
        <div className="titlebar-right">
          ...
          <div className="icons">
            ...
            <div onClick={() => actions.openAboutWindow()} title="About">
              &#xE946;
            </div>
          </div>
        </div>
      ...
      <DashboardPage
        state={state}
        presets={presets}
        visibleChannels={settings.visibleChannels}
        mixerData={mixerData}
